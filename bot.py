...
def find_mob(name: str):
    ...
    pool.sort(
        key=lambda m: (m.get("stars", 0), m.get("level", 0), m.get("health", 0)),
        reverse=True
    )
    return pool[0]
...
async def mob(ctx, *, name: str):
    ...
    general_stats_2 = (
        ...
        f"⏱️ **Attack Spd:** {fmt_num(mob.get('attackSpeed'))}"
    )
    ...
    damage_resist = (
        ...
        f"❄️ **Cold:** {fmt_num(damage.get('cold'))} / {fmt_resist(resist.get('cold'))}\n"
        f"✨ **Magic:** {fmt_num(damage.get('magic'))} / {fmt_resist(resist.get('magic'))}\n"
        f"☠️ **Poison:** {fmt_num(damage.get('poison'))} / {fmt_resist(resist.get('poison'))}\n"
    ...
    evasions = (
        f"🛡️ **Physical:** {fmt_num(mob.get('physicalEvade'))}\n"
